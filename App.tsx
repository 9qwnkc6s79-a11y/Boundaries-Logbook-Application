...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    if (needsMigration) {
      try {
        await db.syncUser(migratedUser, { passwordIsAuthoritative: true });
  ...
  const handleSignupAction = async (user: User) => {
    ...
    await db.syncUser(secureUser, { passwordIsAuthoritative: true });
    const freshData = await performCloudSync();
  ...
  const handlePasswordResetAction = async (email: string, pass: string) => {
    ...
    await db.syncUser(updated, { passwordIsAuthoritative: true });
  ...
  const handleOnboardingComplete = async (data: OnboardingData) => {
    ...
    await db.syncUser(adminUser, { passwordIsAuthoritative: true });
