...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // Save all migrations in a single write. Only assert changePassword when we
    // actually re-hashed — an orgId-only migration must not pass changePassword,
    // or it would bypass syncUser's password-preservation safeguard and let a
    // stale in-memory password clobber the canonical cloud value.
    if (needsMigration) {
      try {
