...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // actually re-hashed — otherwise the stale in-memory hash carried on
    // migratedUser would clobber a password that was changed in the cloud
    // between the sync at the top of this handler and this write (e.g. by an
    // admin reset or a concurrent reset on another device).
    if (needsMigration) {
      try {
