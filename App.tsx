...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    if (needsMigration) {
      try {
        await db.syncUser(migratedUser, { updatePassword: !isHashed(found.password) });
  ...
  const handleSyncToastEmployees = useCallback(async (toastEmployees: ToastSyncEmployee[]) => {
    ...
    for (const emp of toastEmployees) {
      ...
      // updatePassword is true here because this is a brand-new user seeded
      // from Toast — there is no existing password in the cloud to preserve.
