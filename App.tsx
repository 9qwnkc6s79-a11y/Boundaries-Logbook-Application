...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // Save all migrations in a single write. Only pass changePassword=true when
    // we actually hashed a new password — otherwise the stale in-memory hash
    // would bypass syncUser's preservation guard and could clobber a freshly
    // reset password from an admin or another device.
    if (needsMigration) {
      try {
