...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // actually hashed a new value — otherwise migratedUser.password is a stale
    // in-memory hash read at login time, and passing changePassword: true would
    // let syncUser overwrite a password that was reset in the cloud after our
    // read (admin reset from another tab, self-service reset from another
    // device, etc.), reverting the user to their old password.
    if (needsMigration) {
      try {
