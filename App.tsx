...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // Only opt into changePassword when we actually re-hashed the password.
    // Passing changePassword:true for an orgId-only migration would let this
    // login's stale password snapshot overwrite a password an admin reset
    // between our sync and this write — the recurring lockout loop.
    if (needsMigration) {
      try {
