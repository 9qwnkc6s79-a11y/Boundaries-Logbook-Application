...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // actually re-hashed — otherwise syncUser would carry our stale login-time
    // hash through and overwrite a fresh password reset written by another
    // device between the login fetch and this sync, locking the user out with
    // the "my password just changed" symptom.
    if (needsMigration) {
      try {
        await db.syncUser(migratedUser, { changePassword: passwordRehashed });
        console.log(`[Auth] Migrated user ${found.email} (rehashed=${passwordRehashed}, orgId=${!found.orgId})`);
      } catch (e) {
