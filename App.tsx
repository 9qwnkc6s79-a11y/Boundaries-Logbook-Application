...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // Save all migrations in a single write. Only pass changePassword when we
    // actually re-hashed the password — otherwise migratedUser.password is the
    // stale value we read at the top of this handler, and passing
    // changePassword:true would bypass syncUser's preservation guard and let
    // that stale hash silently overwrite a concurrent admin-driven reset from
    // TeamManagement (the "password suddenly changed" bug).
    if (needsMigration) {
      try {
        await db.syncUser(migratedUser, { changePassword: passwordWasRehashed });
        console.log(`[Auth] Migrated user ${found.email} (hash=${passwordWasRehashed}, orgId=${!found.orgId})`);
      } catch (e) {
