...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // Save all migrations in a single write. Only opt in to changePassword
    // when we actually re-hashed the password — otherwise the in-memory hash
    // (which may be stale relative to Firestore after a concurrent reset or
    // a failed pre-login sync) would clobber the fresher cloud hash and lock
    // the user out on their next login.
    if (needsMigration) {
      try {
        await db.syncUser(migratedUser, { changePassword: rehashedPassword });
        console.log(`[Auth] Migrated user ${found.email} (rehashed=${rehashedPassword}, orgIdSet=${!found.orgId})`);
      } catch (e) {
