...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    if (needsMigration) {
      try {
        await db.syncUser(migratedUser, { allowPasswordChange: true });
  ...
  const handleSignupAction = async (user: User) => {
    ...
    await db.syncUser(secureUser, { allowPasswordChange: true });
    const freshData = await performCloudSync();
  ...
  const handlePasswordResetAction = async (email: string, pass: string) => {
    ...
    await db.syncUser(updated, { allowPasswordChange: true });
  ...
  const handleOnboardingComplete = async (data: OnboardingData) => {
    ...
    await db.syncUser(adminUser, { allowPasswordChange: true });
  ...
  const handleSyncToastEmployees = useCallback(async (toastEmployees: ToastSyncEmployee[]) => {
    ...
    for (const emp of toastEmployees) {
      ...
      // allowPasswordChange is safe here: the block above continues if a user
      // already exists for this guid/email, so this only runs for brand-new users.
