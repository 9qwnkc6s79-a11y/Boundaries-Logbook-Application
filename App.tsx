...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // actually rehashed the password — otherwise the stale in-memory hash we
    // loaded at login start would clobber a password that was reset on another
    // device while this login was in flight.
    if (needsMigration) {
      try {
