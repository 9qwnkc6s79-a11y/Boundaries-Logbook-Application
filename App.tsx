...
const App: React.FC = () => {
  ...
  const handleLoginAction = async (email: string, pass: string): Promise<User> => {
    ...
    // Save all migrations in a single write. Only pass changePassword when the
    // password itself was actually re-hashed here — passing it for a pure orgId
    // migration would carry the stale in-memory hash past syncUser's safeguard
    // and silently overwrite a concurrent admin password reset.
    if (needsMigration) {
      try {
