...
class CloudAPI {
  ...
  async syncUser(user: User, options?: { changePassword?: boolean }): Promise<User[]> {
    ...
    // default — for ANY existing value, hashed or plaintext — and require an
    // explicit { changePassword: true } to write a new value. Without this, a
    // stale plaintext password in a caller's in-memory User object would
    // overwrite the canonical cloud password.
