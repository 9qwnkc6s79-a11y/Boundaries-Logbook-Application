  ...
  async syncUser(user: User): Promise<User[]> {
    ...
    try {
      await firestore.runTransaction(async (txn: any) => {
        ...
        // Guard: never downgrade a hashed password to plaintext or empty.
        let toWrite = user;
        ...
        if (existing?.password && isHashed(existing.password)) {
          if (toWrite.password && !isHashed(toWrite.password)) {
            console.warn(`[Firestore] syncUser: BLOCKED plaintext overwrite for ${user.email} — keeping hashed version`);
